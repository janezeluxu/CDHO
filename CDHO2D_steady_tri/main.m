function [Error] = main(maxIter,fileName,output_filename,solutionFile,GaTol)
...
[OrderList,meshData,vertexData,Area,...
    IBC,IBC_vertex,BCval,BCb,uHBCE,solution_ien,order_list,edge_usage,...
...
if itau == 3
    ...
    dt = 3/7;
    [tau] = startingPoint(patchArea,patchOrder,vertexData);
    gGlobal = zeros(TotalDOF,1);
    for iteration = 1:maxIter
        ...
        [taunew] = solvetauHOmap(OrderList,vertexData,patchArea,patchOrder,...
            IBC_vertex,BCb,meshData,solution_ien,order_list,u,uHBCE);
        %[taunew] = solvec1(OrderList,vertexData,Area,IBC,uHBCE,1,meshData,solution_ien,order_list,u);
...
%[H1]=getErrorwithRef(fileName,solutionFile);
%% write the solution in vtk form
writeMesh(meshData,vertexData,'boundary')
writeSolution(u,solutionFile)
writevtkfile(meshData,vertexData,u(1:node_Size),tau(1:node_Size),zeros(Grid_size,1));
...
function [taum] = startingPoint(patchArea,patchOrder,vertexData)
global node_Size;
global kappa;
global a;
