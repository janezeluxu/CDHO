function twod_to_vtk ( xy, e_conn, uvp,celldata, output_filename, title )
...
%
  ...
  cell_data = zeros ( 1, element_num );
  cell_data(1,1:element_num) = celldata(1:element_num)';
...
%
  vtk_puv_write ( output_unit, title, node_num, element_num, ...
    element_order2, xyz, element_node, concentration,cell_data, ga);
...
function vtk_puv_write ( output_unit, title, node_num, element_num, ...
  element_order, xyz, element_node, concentration,cell_data,ga )
...
%
  ...
  fprintf ( output_unit, 'SCALARS error double\n' );
  fprintf ( output_unit, 'LOOKUP_TABLE default\n' );
  for element = 1 : element_num
    fprintf ( output_unit, '  %2.16f\n', cell_data(1,element) );
  end
