...
elseif casenumber == 300
    % case convection of a pulse
    a = {@(x,y) -y+0.5, @(x,y) x-0.5};
    kappa = [1,0;0,1]*1e-5;
    force = 0;
    nstp = floor(20*pi);
    dt = 1/20;
    
    Dirichlet_BC{1} = [2;5;16;18;6;14;23;24];
    Dirichlet_BC{2} = {0;0;0;0;0;0;0;0};
    Dirichlet_edge = [6;14;23;24];
    static_factor = 9;
    master = [];
    slave = [];
    casefile = "./pulse/";
    order = 'p1';
    meshFile = strcat('./mesh/box',order,'.txt')
    output_filename = strcat('./testvtk/cases/',casefile,order,itaucase,'.vtk')
    solutionFile = strcat('./testvtk/cases/',casefile,'solution/',order,itaucase,string(nstp),'.txt')
    main(meshFile,output_filename,solutionFile,0.5);
    plotSolution(meshFile,solutionFile,1);
elseif casenumber == 400
    tic
    % case verification
    vmax = 500;
    a = {@(x,y) -y*(y-1)*vmax,@(x,y) 0};
    kappa = [1,0;0,1]*1e-2;
    force = 0;
    nstp = 100;
    dt = 1e-4;
    u_in = 10;
    Dirichlet_BC{1} = [2;6;5;23;16;18;24];
    Dirichlet_BC{2} = {0;u_in;0;0;0;0;0};
    
    Dirichlet_edge = [23,24];
    static_factor = 9;
    %master = [2;24;18];
    %slave = [5;23;16];
    casefile = "./plate/";
    order = 'p1';
    meshFile = strcat('./mesh/plate',order,'.txt')
    output_filename = strcat('./testvtk/cases/',casefile,order,itaucase,'.vtk')
    solutionFile = strcat('./testvtk/cases/',casefile,'solution/',order,itaucase,'.txt')
    main(meshFile,output_filename,solutionFile,vmax);
    toc
%     figure(2)   
%     y = 0:0.01:1;
%     x = 0.79*ones(length(y),1);
%     concentration = LinePlot(meshFile,solutionFile,x,y);
%     Lineplotfilename = strcat('./testvtk/cases/',casefile,'solution/',order,itaucase,'line.txt');
%     writeSolution(concentration,Lineplotfilename)
