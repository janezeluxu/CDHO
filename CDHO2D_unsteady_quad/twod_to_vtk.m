function twod_to_vtk ( xy, e_conn, uvp,taum, output_filename, title )
...
%
  ...
  concentration = zeros ( 1, node_num );
  concentration(1,1:node_num) = uvp(1:node_num,1)';
  
  tau = zeros ( 1, element_num );
  tau(1,1:element_num) = taum(1:element_num)';

  ga = zeros ( 1, node_num );
  ga(1,1:node_num) = uvp(1:node_num,3)';
...
%
  vtk_puv_write ( output_unit, title, node_num, element_num, ...
    element_order2, xyz, element_node, concentration,tau, ga);
...
function vtk_puv_write ( output_unit, title, node_num, element_num, ...
  element_order, xyz, element_node, concentration,tau,ga )
...
%
  ...
  fprintf ( output_unit, 'SCALARS ut double\n' );
  fprintf ( output_unit, 'LOOKUP_TABLE default\n' ); 
  for node = 1 : node_num
    fprintf ( output_unit, '  %2.16f\n', ga(1,node) );
  end
...
fprintf ( output_unit, 'CELL_DATA %d\n', element_num );
  fprintf ( output_unit, 'SCALARS taum double\n' );
  fprintf ( output_unit, 'LOOKUP_TABLE default\n' );
  for element = 1 : element_num
    fprintf ( output_unit, '  %2.16f\n', tau(1,element) );
  end
